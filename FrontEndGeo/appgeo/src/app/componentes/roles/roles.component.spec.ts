...
describe('ComponenteRoles', () => {
  ...
  it('Debe insertar un nuevo rol', async () => {
    ...
    componente.form.addControl('permisos', (componente as any).fb.array([]));
    permisosMock.forEach(id => componente.permisos.push(new FormControl(id)));
    componente.form.patchValue({ nombre: 'NUEVO ROL', permisos: permisosMock }); 
  ...
  it('Debe actualizar un rol existente', async () => {
    ...
    componente.form.addControl('permisos', (componente as any).fb.array([])); 
    permisosMock.forEach(id => componente.permisos.push(new FormControl(id)));
    componente.form.patchValue({ nombre: 'ROL ACTUALIZADO', permisos: permisosMock }); 
    componente.id = '1';
    await componente.Guardar();
